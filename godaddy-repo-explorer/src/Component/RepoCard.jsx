...
function RepoCard({repo , index , handleDrawer , openDrawer}) {
  try{
      return (
        <div onClick={()=>{handleDrawer(repo)}} key={index} className={` hover:shadow-md transition-all duration-300 cursor-pointer w-full sm:w-[48%] lg:w-[32%]  min-h-[auto] max-h-[60px] overflow-y-hidden p-1 rounded-md ${openDrawer.id == repo.id ? 'outline outline-3 outline-[var(--godaddy-blue-100)]': 'border-2 '}`}>
